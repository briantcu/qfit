class SubscriptionsController < ApplicationController
  ...
  before_filter :verify_logged_in, except: [:stripe_event, :activate]
  ...
  def activate
    ...
    result = SubscriptionService.instance.activate_subscription_for_apple(current_user, receipt_data)
    if result[:status] == 'success'
      render status: 201, json: {status: 'success', message: 'Subscription Created!'}
    else
      render status: 422, json: {message: result[:message], status: 'failed'}
