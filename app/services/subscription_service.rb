...
class SubscriptionService
  ...
  def activate_subscription_for_apple(user, receipt)
    response = {status: 'success', message: 'none'}

    begin
      opts = { shared_secret: '3328dbd9f8ec4040b9e80b43011e6462' }

      #@TODO this defaults to prod, then retries for sandbox depending on apple response
      if apple_receipt = Venice::Receipt.verify(receipt, opts)
        expires_at = apple_receipt.expires_at
      end
    rescue => e
      Rails.logger.info('Apple - error checking out for user ' + user.id.to_s)
      Rails.logger.info(e)
      Rails.logger.info(receipt)
      Rails.logger.info(apple_receipt)
      Rollbar.error(e)
      response[:status] = 'failed'
      response[:message] = "Sorry! Apple subscription sync failed."
    else
      #@TODO compare expires_at with now?
      user.update!(active_until: expires_at.to_datetime, paid_tier: 2, status: 1)
      EmailService.perform_async(:apple_billing_happened)
    end
    response
  end
