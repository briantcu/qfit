...
class RoutineService
  ...
  def self.nightly_workout_creation
    begin
      ...
      users.each do |user|
        RoutineService.new(user, 'CRON', nil).create_routines
        Rails.logger.info('workouts created for user ' + user.email)
      end
  ...
  def create_routine
    ...
    @entity.transaction do
      ...
      Rails.logger.info('creating routine for entity id ' + @entity.id.to_s)
      ...
      if @routine.present? # Regular scheduled workout
        ...
        if @entity.is_group?
          last_weeks_routine = GroupRoutine.find_by(day_performed: @date - 7.days, program_day_id: 0, changes_saved: true, group_id: @entity.id)
        else
          last_weeks_routine = DailyRoutine.find_by(day_performed: @date - 7.days, program_day_id: 0, changes_saved: true, user_id: @entity.id)
        end
