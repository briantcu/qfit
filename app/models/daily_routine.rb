...
class DailyRoutine < ActiveRecord::Base
  ...
  validates :day_performed, presence: true, uniqueness: { scope: :user_id}
  ...
  def self.get_matching_routines(routine)
    DailyRoutine.where(:wt_day_id => routine.wt_day_id, :sp_day_id => routine.sp_day_id, :pl_day_id => routine.pl_day_id,
                       :wu_day_id => routine.wu_day_id, :user_id => routine.user.id).order(id: :desc).limit(5)
  end
  ...
  def self.get_routine_by_date(month, year, day, user_id)
    ...
    DailyRoutine.includes(:performed_exercises, :performed_plyometrics, :performed_sprints, :performed_warm_ups).where(day_performed: date, user_id: user_id).first
  end
