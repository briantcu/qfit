...
var RoutineActions = {
    ...
    addExercise: function(routineId, type, exId) {
        if (!routineId) {
            this.createRoutine(type, exId, this.addExercise);
            return
        }
        if (gon.viewing == 'user') {
    ...
    createRoutine(type, exId, callback) {
    ...
    addCustomExercise: function(routineId, type, name) {
        if (!routineId) {
            this.createRoutine(type, name, this.addCustomExercise);
            return
        }
        var type_id = 4;
        if (type == 'weights') {
