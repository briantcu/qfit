...
RSpec.describe MobileFbSignInController, type: :controller do
  ...
  it 'should create a new user account' do
    allow_any_instance_of(FbGraph2::User).to receive(:fetch).and_return(true)
    post(:sign_in_up, email: 'a@b.com', uid: '124232', first_name: 'brian', last_name: 'regan', user_name: 'username', image: 'thisistheimage', gender: 'male', fb_token: 'alkdjflka')
  ...
  it 'should log in an existing user' do
    ...
    post(:sign_in_up, email: 'a@b.com', uid: '12367489', first_name: 'brian', last_name: 'regan', user_name: 'username', image: 'thisistheimage', gender: 'male', fb_token: 'alkdjflka')
    expect(response.status).to eq(200)
    body = JSON.parse(response.body)
  ...
  it 'should not create regular user with invalid email' do
    allow_any_instance_of(FbGraph2::User).to receive(:fetch).and_return(true)
    post(:sign_in_up, email: 'ab.com', uid: '12367489', first_name: 'brian', last_name: 'regan', user_name: 'username', image: 'thisistheimage', gender: 'male', fb_token: 'alkdjflka')
